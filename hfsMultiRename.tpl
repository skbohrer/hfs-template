...
[box actions]	
	<fieldset id='actions'>
		...
		<center>
		{.if|{.can rename.}|
			<button id='chgIdBtn' onclick='groupRename.call(this)'>Group Rename</button>
...
function groupRename() {
	var a = selectedItems();
	if (a.length < 1) {
		return alert("You must select one or more Shift files for rename");
	}
	ezprompt("Specify starting char pos, and the num chars to replace, then the replacement str.", {"type":"text", "default":"0 3 STR"}, function(idStr){
		var fileAry = [], strsAry = [];
		idStr = idStr.trim();
		strsAry = idStr.split(" ");
		a.each(function(index, elem){
			return changeShiftName(fileAry, elem, strsAry[2], +strsAry[0], +strsAry[1]);
		});
		if (fileAry.length) {	// some files to rename
			putMsg('{.!Renaming with "' + strsAry[2] + '" on ' + fileAry.length + ' shift files.}');
			setTimeout(function() {renameMultipleFiles(fileAry);}, 10);
		}
	});
}//groupRename
